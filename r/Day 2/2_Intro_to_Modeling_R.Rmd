...
cpi <- readr::read_csv(here("..", "data", "food_cpi.csv"))
...
gasoline_lag = Gasoline - lag(Gasoline)
gasoline_change = (Gasoline-lag(Gasoline)/lag(Gasoline))

consumables_cpi_mutate <- mutate(
  consumables_cpi, gasoline_lag,
  gasoline_change_percentage = 0.1*gasoline_chnage),
  
  a_t_c_change = Alcohol_tobacco_cannabis - lag(Alcohol_tobacco_cannabis), 
a_t_c_change_perc = 0.1*(Alcohol_tobacco_cannabis - lag(Alcohol_tobacco_cannabis)/lag(Alcohol_tobacco_cannabis)),

energy_change = Energy - lag(Energy),
energy_change_perc = 0.1*(Energy-lag(Energy)/lag(Energy)))
...
filter(food_cpi_mutate, (food_change < All_items_but_food_energy) & (food_change > 1))
