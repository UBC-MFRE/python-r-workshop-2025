...
Create `sum`, equal to 0. Take the numbers from 1 to 20. For each odd number from 1 to 20, add it to `sum`, and multiply sum by every even number. (0+1=1
3+0=3
)
...
```{r}
n <- as.integer(readline(prompt = "Enter the upper limit: "))
...
for (i in 1:n) {
  if (i %% 2 == 1) {
    new_sum <- sum_val + i
    print(sprintf("%d + %d = %d", sum_val, i, new_sum))
  } else {
    new_sum <- sum_val * i
    print(sprintf("%d * %d = %d", sum_val, i, new_sum))
  }
...
#Write a function in R to convert F to C, use user input to take the value of F. 
...
f<- as.numeric(readline(prompt = "Enter Temp in F:"))
value<-f_to_c(f)
print(value)
...
calculate_correlation <- function(x, y) {
  ...
  corr_a_b <- var_e / sqrt(var_c * var_d)
...
#1: Write a R code to inverse an array. take user input for the array
10,12,13,14,15
15,14,13,12,10

#2: Write a R code to create a function called "factorial" and calculate factorial of user given input of n. 
...
#take user input the array elements
for(i in 1:n){
  array[i]<-as.numeric(readline())
}
...
print(paste("Factorial of", n, "is", factorail_func(n)))
